...
//
// Generated on: 2017-04-16 22:34:43
//
// -------------------------------------------------------------
...
//   Overflow Mode   : saturate
// -------------------------------------------------------------
  parameter CLKRATE = 9; //uint32
  parameter LATENCY = 9; //uint32
  parameter INITIAL_LATENCY = 19; //uint32
  parameter NO_OF_TESTS = 3149; //uint32
  parameter MAX_TIMEOUT = 82; //uint32
  ...
  always @(posedge clk or negedge rst_n) //tb_enb_gen
  begin
    if (~rst_n) begin 
       // Nothing to reset.
    end 
  ...
  always @(posedge clk or negedge rst_n)
  begin
    filter_in_data_log_task(clk,rst_n,
  ...
  assign expected_ce_out =  delayLine_out & we;
...
// ---- Bypass Register ----
  ...
  always @ (posedge clk or negedge rst_n) // checker_filter_out
  begin
    if (rst_n == 0) begin 
  ...
  always @ (posedge clk or negedge rst_n) // checkDone_1
  begin
    if (rst_n == 0)
      check1_Done <= 0;
